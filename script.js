...
const ICONS = {
    ...
    reddit: '<path d="M24 11.5c0-1.65-1.35-3-3-3-.96 0-1.86.48-2.42 1.24-2.22-1.61-5.35-2.6-8.85-2.63l1.52-7.14 5.12 1.07c.12.98.95 1.75 1.98 1.75 1.1 0 2-.9 2-2s-.9-2-2-2c-.93 0-1.73.65-1.95 1.52l-5.55-1.16c-.22-.05-.44.07-.51.28l-1.77 8.3c-3.62.13-6.85 1.19-9.12 2.85-.56-.76-1.46-1.24-2.42-1.24-1.65 0-3 1.35-3 3 0 1.33.85 2.47 2.03 2.86-.06.46-.1.93-.1 1.41 0 3.86 4.48 7 10 7s10-3.14 10-7c0-.48-.04-.95-.1-1.41 1.18-.4 2.03-1.53 2.03-2.86zm-16.5 2.5c.83 0 1.5.67 1.5 1.5s-.67 1.5-1.5 1.5-1.5-.67-1.5-1.5.67-1.5 1.5-1.5zm6.5 7.5c-2.21 0-4.05-1.1-4.73-2.62.46-.19.93.07 1.12.54.43 1.02 1.74 1.68 3.61 1.68 1.86 0 3.17-.66 3.6-1.68.19-.46.68-.71 1.14-.54.67 1.52-1.17 2.62-4.74 2.62zm4-6c-.83 0-1.5.67-1.5 1.5s.67 1.5 1.5 1.5 1.5-.67 1.5-1.5-.67-1.5-1.5-1.5z"/>',
    bandcamp: '<path d="M0 18.538h8.545l7.143-13.076h-8.546zM24 5.462h-8.545l-7.143 13.076h8.546z"/>',
...
function createWindow(path, html) {
    ...
    // Process Links into Buttons with Icons
    processLinks(content);
    
    windowDiv.appendChild(header);
    windowDiv.appendChild(content);
    desktopArea.appendChild(windowDiv);
}
...
function processLinks(container) {
    ...
    links.forEach(link => {
        ...
        else if (url.includes('github') || url.includes('namthedev')) iconSvg = ICONS.github;
        else if (url.includes('twitter') || url.includes('x.com')) iconSvg = ICONS.twitter;
